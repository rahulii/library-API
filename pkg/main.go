package main

import (
	"database/sql"
	"encoding/json"
	"fmt"
	"log"
	"net/http"
	"os"
	_ "github.com/go-sql-driver/mysql"
	"github.com/gorilla/mux"
)


const (
	API_PATH = "/api/v1/books"
)

type library struct {
	dbHost,dbPass,dbName string
}

type book struct {
	Id,Name,Isbn string
}

func main() {
	dbHost := os.Getenv("DB_HOST")
	if dbHost == "" {
		dbHost = "localhost:3306"
	}

	dbPass := "password"
	// if dbPass == "" {
	// 	dbPass = "admin@1234"
	// }

	apiPath := os.Getenv("API_PATH")
	if apiPath == "" {
		apiPath = API_PATH
	}

	dbName := os.Getenv("DB_NAME")
	if dbName =="" {
		dbName = "library"
	}
	l := library{
		dbHost: dbHost,
		dbPass: dbPass,
		dbName: dbName,
	}
	r := mux.NewRouter()
	r.HandleFunc(apiPath,l.getBooks).Methods(http.MethodGet)
	r.HandleFunc(apiPath , l.postBooks).Methods(http.MethodPost)
	http.ListenAndServe(":8000",r)
}

func (l library) postBooks(w http.ResponseWriter , r *http.Request) {
	//read the request into an instance of book
	 Book := book{}
	 json.NewDecoder(r.Body).Decode(&Book)
	//open the connection
	db := l.openDbConnection()
	//write the data
	insertQuery,err := db.Prepare("insert into books values (?,?,?)")
	if err!=nil {
		log.Fatalf("preparing query %s\n",err.Error())
	}
	tx , err := db.Begin()
	if err!=nil {
		log.Fatalf("while beginning error %s\n",err.Error())
	}
	_,err = tx.Stmt(insertQuery).Exec(Book.Id,Book.Name,Book.Isbn)
	if err!=nil {
		log.Fatalf("while inserting values %s\n",err.Error())
	}
	err = tx.Commit()
	if err!=nil {
		log.Fatalf("commiting the transaction%s\n",err.Error())
	}
	//close the connection
	l.closeDbConnection(db)

}

func (l library) getBooks(w http.ResponseWriter, r *http.Request) {
	//open connection
	db:= l.openDbConnection()
	//read all the books
	rows,err := db.Query("SELECT * from books")
	if err != nil {
		log.Fatalf("Error reading %s\n",err.Error())
	}
	var books []book
	for rows.Next() {
		var id,name,isbn string
		rows.Scan(&id,&name,&isbn)
		if err != nil {
			log.Fatalf("Error while scanning row%s\n",err.Error())
		}
		abook := book{
			Id: id,
			Name: name,
			Isbn: isbn,
		}
		books = append(books, abook)
	}
	json.NewEncoder(w).Encode(books)

	// close the connection
	l.closeDbConnection(db)

}

func (l library) openDbConnection() *sql.DB {
	db,err:= sql.Open("mysql",fmt.Sprintf("%s:%s@(%s)/%s","newuser",l.dbPass,l.dbHost,l.dbName))
	if err!=nil {
		log.Fatalf("Error:%s",err.Error())
	}
	return db
}

func (l library)closeDbConnection(db *sql.DB) {
	err:= db.Close()
	if err != nil {
		log.Fatal("while clossing the connection %s\n",err.Error())
	}
}